...
- **HELP consistency test set.** New checks under `test/index.test.js` verify every command exports a `HELP` string starting with `draftwise <cmd>` and that key concepts in the per-command HELP also appear in the top-level listing. Catches drift without runtime cost. — Ankur (#TBD)
- **`stream.abort()` on errors** in `src/ai/providers/claude.js`. The streaming branch now wraps iteration in `try/catch`; if iteration or `onToken` throws, the underlying HTTP request is closed before the error propagates. — Ankur (#TBD)
- **`mapConcurrent` no-cancel doc note.** Header comment in `src/utils/concurrency.js` calls out that workers don't bail on first throw — fine for the scanner use case (errors swallowed inside `fn`), worth knowing if reused elsewhere. — Ankur (#TBD)
...
### Changed
- **Informational `log()` output moved to stderr.** Every command except `list` and `show` now defaults its log injection to `console.error`. `list` and `show` keep `console.log` because their output IS the deliverable (the table, the spec content). The fix lets `draftwise scan > overview.md` actually capture only the streamed content instead of mixing in "API mode — calling claude..." status lines. — Ankur (#TBD)
- **Document the dep-pinning asymmetry in CLAUDE.md.** `@anthropic-ai/sdk` is exact-pinned because it's 0.x; `@inquirer/prompts` and `yaml` use caret because they're stable 1.x+ where minor follows semver. Intentional, not an oversight; Dependabot covers both. — Ankur (#TBD)
