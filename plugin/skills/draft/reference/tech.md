...
- **A product spec exists for this feature?** Run `/draft list` mentally (or peek at `.draftwise/specs/`) — if no `product-spec.md` exists for the slug the user wants, point at `/draft new "<idea>"` first. The tech spec grounds in the product spec; without one, there's nothing to ground in.
...
- **api mode success** (streamed markdown ending with "Wrote .draftwise/specs/<slug>/technical-spec.md"): the CLI wrote it. Confirm and surface the `Next:` step (`/draft tasks`).

- **Multiple specs, no slug given** (error: "Multiple product specs exist…"): show the available slugs and ask which one. Re-invoke `!`draft tech <slug>``.
...
- **No product specs yet** (error: "No product specs found"): point at `/draft new "<idea>"`.
